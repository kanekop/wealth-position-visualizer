...
import { calculateWealthPosition, parseNumberInput } from './utils/calculations';
...
function AppContent() {
  ...
  const handleCalculate = (e) => {
    ...
    // 入力値をパース
    const annualIncome = parseNumberInput(income);
    const totalAssets = parseNumberInput(assets);

    // デバッグ用ログ
    console.log('入力値:', { annualIncome, totalAssets });

    // 実際の計算を実行
    const calculationResults = calculateWealthPosition(annualIncome, totalAssets);
            ...
            <div className="space-y-4">
              <div className="border-b pb-4">
                ...
                <p className="text-2xl font-bold text-primary-600">
                  {t('results.topPercent').replace('{percent}', results.japan.percentile.toFixed(1))}
                </p>
                ...
                <p className="text-xs text-gray-400 mt-2">
                  推定順位: {results.japan.estimatedRank?.toLocaleString()}位
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-primary-600">
                  {t('results.topPercent').replace('{percent}', results.world.percentile.toFixed(1))}
                </p>
                <p className="text-sm text-gray-600">
                  {t('results.wealthShare')
                    .replace('{region}', '世界')
                ...
                {/* デバッグ情報 */}
                <p className="text-xs text-gray-400 mt-2">
                  推定順位: {results.world.estimatedRank?.toLocaleString()}位
                </p>
              </div>
            </div>
          </div>
